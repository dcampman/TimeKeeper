...
class ELGSDPlugin {
  ...
  async loadLocalization(pathPrefix) {
    ...
    this.localizationLoaded = this.readJson(
      `${pathPrefix}${this.language}.json`
    );
...
class ELGSDApi extends ELGSDPlugin {
  ...
  connect(port, uuid, messageType, appInfoString, actionString) {
    ...
    this.websocket.onerror = (evt) => {
      const error = `WEBSOCKET ERROR: ${evt}, ${evt.data}, ${
        SocketErrors[evt?.code]
      }`;
      console.warn(error);
  ...
  send(context, event, payload = {}) {
    this.websocket &&
      this.websocket.send(JSON.stringify({ context, event, ...payload }));
  }
